...
*** Adding Package's Repository
    ...
    by the installation script, e.g, to add 'function-args' packages,
    ...
    If a different source then melpa is required, uses the
    :git keyword, example:
...
*** Installing Packages
    ...
    - Tip: When problems with missing dependencies were encountered
           (happens usually when I'm using another source
           keyword like :git), put the dependency code snippet
           ~(depends-on "missing-dep-package-name")
           above the target package and give `cask install`
           command another shot.
...
*** Removing Packages
    ...
    - TIP: Exploration path: ~~/.emacs.d/.cask/<emacs-version>/<source>~
