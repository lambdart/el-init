...
* Introduction
  ...
  * The same interface is used to program and evaluate.
  ...
  dialect of =LISP=, all we do (inside =Emacs=)
  ...
  current line. Note the desire feature is to move the cursor
  to the beginning of the line, after the evaluation of the
  list in question the data transformation that occurs is
  related to display the cursor in a different position.

  * Features are side effects of data transformations and sometimes
    the computation to perform a particular transformation as
  ...
  dialect called =Emacs Lisp=, usually abbreviated to
  ...
  That way is more clear to the reader that Emacs is not
  ...
  That's why this configuration can be see (IMHO)
  as an =Emacs/Elisp= distribution.
...
* FAQ
  ...
  - A: Maybe, the =concept= of the lisp machine as the main interface
    ...
    more difficult that has to be, and one day - if we don't do
