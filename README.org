...
* Disclaimer
  ...
  and close (crazy) friends.

  So, here goes a little bit of advice: read =Emacs/Elisp= manual,
  using as a exercise the write of your own ~init.el~, but feel
...
** Features:
   ...
   - The same editing capabilities for all applications.
   - The same =GUI Interface= (frames/window/buffers) for all applications.

   - Just one development environment for all projects (independent of
     the programming language and tools needed to construct it):
     - Apparence
     - Editing
     - Documentation
     - Communication
   ...
   - Communication (=pipes, IPCs, data streams=) between peers are
     transparent, resulting in a very practical, very convenient
     integration for all modules and resources available:
     - Packages
     - Applications
...
* Packages
  ...
  Packages are installed as git sub-modules are listed on the
  ~.gitmodules~ file.
