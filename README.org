...
* Disclaimer
  ...
  author himself (and some crazy friends that insists in use this
  peace of shit! Thanks guys, really appreciate all the suggestions,
  and criticism, much love and freedom in your lives)!

  So, here goes a little bit of advice, for everyone: read
  =Emacs/Elisp= manual, and use as a exercise writing your
  own ~init.el~, but feel free to fork or copy anything from
  ~src/eos.org~ that you like or find useful!
  d(-_-)b - nice song, right?!

  Its better to understand, once for all, that as an Emacs
  ...
  So, maybe Emacs its not for you, no problem, move on :)
...
* Introduction
  ...
  But as soon I start to write =Elisp= code to extend it, the answer
  pops on my mind and the frustration removed, and now in a
  =conceptual level= admiration is the only sentiment that as left.

  So, IMO =Emacs= should be explained in the following terms:
  ...
  The aim of this project is to provide an organized configuration
  for =Emacs= to provide a generic =IO= bridge between the human mind
  and the machine resources, that are managed, of course, by the
