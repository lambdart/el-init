...
(defun eos/funcall (func &rest args)
...
(when (require 'exwm nil t)
  (progn
    ...
    ;; enable exwm, todo: move this to .xinitrc
    (exwm-enable)))
...
;; ;; eval after which-key-mode load
...
(defun eos/launch/urxvt ()
  "Launch urxvt"
  (interactive)
  (eos/run/async-proc "urxvt"))

(global-set-key (kbd "C-x ]") 'eos/launch/urxvt)
...
(when (require 'dash-docs nil t)
  (progn
    (define-key eos-docs-map (kbd "u")
      'dash-docs-update-docset)))

;; auxiliary function
(defun eos/dash/set-docset (docset)
  "Set dash DOCSET."
  (if (file-exists-p (concat (dash-docs-docsets-path) "/" docset ".docset"))
      (eos/funcall 'helm-dash-activate-docset docset)))
