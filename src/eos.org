...
* EOS
  ...
  Emacs OS - an org-mode based =Elisp= distribution,
  using =Emacs= as the only IO bridge between human (mind)
  and the machine (resources).
...
*** Header
    ...
    ;;; init.el --- eos.org: an org-mode based Elisp distribution
...
**** Keymaps
     ...
     Basically we creates new prefix maps, all the must used commands
     from vanilla Emacs or some 3party packages (major/minor) modes
     will be put under this keymaps.
        ...
        (lambda ()
          ...
          (let ((minibuffer (active-minibuffer-window)))
            (when minibuffer
              ...
              (when (fboundp 'minibuffer-keyboard-quit)
...
***** Hooks
      ...
      ;; (funcall 'dashboard-setup-startup-hook)
      ;; (add-hook 'window-setup-hook
                ;; (lambda ()
                ;;   (funcall 'dashboard-insert-startupify-lists)
                ;;   (eos/dashboard/open-buffer))
...
** Lab
   ...
   To add more experiments to this already bloated
   configuration edit ~emacs.d/lisp/lab/lab.el~.
