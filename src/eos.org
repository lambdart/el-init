...
#+DESCRIPTION: an org-mode based Emacs distribution
#+KEYWORDS: emacs, eos, emacs config, init.el
#+LANGUAGE: en
#+BABEL: :cache yes
...
* EOS
  ...
  Using =Emacs= as the only dynamic interface
...
*** Header
    ...
    ;;; init.el --- an org-mode based Elisp distribution
...
**** Binds
     ...
     ;; eos prefixs
     (define-key ctl-x-map (kbd "f") 'eos-files-map) ;; files
     ;; (define-key ctl-x-map (kbd "") 'eos-filter-map)
     (define-key ctl-x-map (kbd "p") 'eos-pm-map) ; project
     (define-key ctl-x-map (kbd "t") 'eos-tags-map) ; tags
     (define-key ctl-x-map (kbd "c") 'eos-utils-map) ; commands
     (define-key ctl-x-map (kbd "e") 'eos-sc-map) ; errors
     (define-key ctl-x-map (kbd "l") 'eos-docs-map) ; library
     (define-key ctl-x-map (kbd "v") 'eos-media-map) ; media
     (define-key ctl-x-map (kbd "<tab>") 'eos-completion-map) ; tab - complete
...
*** Load-path
    ...
    Uses ~subdirs.el~ and $EMACSLOADPATH environment variable
...
***** Binds
      ...
      (define-key eos-utils-map (kbd "g") 'mark-goto-char)
...
***** Binds
      ...
      (define-key eos-files-map (kbd "t") 'recentf-find-file)
...
****** Binds
       ...
       (define-key ctl-x-map (kbd "<C-return>") 'eos-term)
...
***** Binds
      ...
      ;; binds eos-media-map
      (define-key eos-media-map (kbd "P") 'play-playlist)
      (define-key eos-media-map (kbd "a") 'emms-add-file)
      (define-key eos-media-map (kbd "s") 'emms-start)
      (define-key eos-media-map (kbd "q") 'emms-stop)
      (define-key eos-media-map (kbd "v") 'emms-pause)
      (define-key eos-media-map (kbd "p") 'emms-previous)
      (define-key eos-media-map (kbd "n") 'emms-next)
      (define-key eos-media-map (kbd "r") 'emms-random)
...
***** Functions
      ...
      (defun eos-initialize-dashboard ()
...
***** Enable
      ...
      (add-hook 'after-init-hook 'eos-initialize-dashboard)
...
***** Functions
      ...
      (defun eos-dictionary-search-at-point ()
        ...
        (let ((word (thing-at-point 'word)))
          ;; asks for a word if none was found at point
          (unless word
...
***** Binds
      ...
      (define-key eos-docs-map (kbd "l") 'dash-docs-search-docs)
...
***** Binds
      ...
      ;; eos-complete map
      (define-key eos-completion-map (kbd "`") 'company-ispell)
      (define-key eos-completion-map (kbd "f") 'company-files)
      (define-key eos-completion-map (kbd "e") 'company-etags)
      (define-key eos-completion-map (kbd "y") 'company-yasnippet)

      ;; redundancy
      (define-key eos-completion-map (kbd "TAB") 'eos-complete-in-buffer-or-indent)

      ;; global
      (global-set-key (kbd "C-TAB") 'eos-complete-in-buffer-or-indent)
...
***** Functions
      ...
      (defvar eos-site-loaddefs
        (concat
         (expand-file-name "site-lisp/" user-emacs-directory) "site-lisp-loaddefs.el")
        "Default site-lisp-loaddefs file.")

      (defvar eos-lisp-loaddefs
        (concat
         (expand-file-name "lisp/" user-emacs-directory) "lisp-loaddefs.el")
        "Default lisp-loaddefs file.")

      (defun eos-update-packages-autoloads ()
        "Update packages autoloads using Vex `update-packages-autoloads'."
        (interactive)
        (let ((files (list eos-site-loaddefs eos-lisp-loaddefs)))
          ;; update lisp autoloads
          (dolist (file files)
            (update-packages-autoloads
      ...
      (defun eos-compile ()
        ...
        (interactive)
        (compile-at-dir
         (expand-file-name user-emacs-directory) eos-compile-command))
...
***** Binds
      ...
      ;; utils map
      (define-key eos-utils-map (kbd "c") 'compile-history)
...
***** Binds
      ...
      (define-key prog-mode-map (kbd "TAB") 'eos-complete-in-buffer-or-indent)
