...
**** Enable
     ...
     (add-hook 'window-setup-hook
               (lambda ()
                 (safe-set-frame-font "Iosevka Fixed Extended-14")))
...
***** Binds
      ...
      ;; ctl-x-map
      (define-key ctl-x-map (kbd "C-c") 'execute-command-history)

      ;; (define-key vlm-utils-map (kbd "e") 'execute-command-history)
      ...
      (eval-after-load 'icomplete
        (lambda ()
          (when (boundp 'icomplete-minibuffer-map)
            ...
            (define-key icomplete-minibuffer-map (kbd "M-k") 'icomplete-kill-candidate)
            (define-key icomplete-minibuffer-map (kbd "M-i") 'icomplete-insert-candidate))))
